...
# -----------------------------------------------------------------------------
# Fleet Enroll Secrets - Fetch from API and store in Secrets Manager
# -----------------------------------------------------------------------------

# Secret to store all team enroll secrets as JSON
resource "aws_secretsmanager_secret" "fleet_enroll_secrets" {
  count                   = var.fetch_enroll_secrets ? 1 : 0
  name                    = "fleet-enroll-secrets"
  recovery_window_in_days = 0
}

# Fetch enroll secrets from Fleet API and store in Secrets Manager
resource "null_resource" "fetch_enroll_secrets" {
  count = var.fetch_enroll_secrets ? 1 : 0

  triggers = {
    # Re-run when Fleet service updates or on demand
    service_updated = module.ecs_service_fleet.task_definition_arn
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOF
      ...
      # Wait for Fleet to be healthy
      echo "Waiting for Fleet to be healthy..."
      for i in {1..30}; do
        if curl -sf "https://${var.domain_name}/healthz" > /dev/null 2>&1; then
      ...
      FLEET_API_TOKEN=$(aws secretsmanager get-secret-value \
        --secret-id "${var.fleet_api_key_secret_name}" \
        --query SecretString --output text)

      # Fetch global enroll secret
      GLOBAL_SECRET=$(curl -sf "https://${var.domain_name}/api/v1/fleet/spec/enroll_secret" \
        -H "Authorization: Bearer $FLEET_API_TOKEN" | jq -r '.spec.secrets[0].secret // empty')

      # Fetch all teams and their secrets
      TEAMS_JSON=$(curl -sf "https://${var.domain_name}/api/v1/fleet/teams" \
        -H "Authorization: Bearer $FLEET_API_TOKEN")

      # Build JSON object with all secrets
      SECRETS_JSON=$(echo "$TEAMS_JSON" | jq -c --arg global "$GLOBAL_SECRET" '
        {
          FLEET_GLOBAL_ENROLL_SECRET: $global
        } + (
          .teams | map({
            key: ("FLEET_" + (.name | gsub(" "; "_") | gsub("-"; "_") | ascii_upcase) + "_ENROLL_SECRET"),
      ...
      aws secretsmanager put-secret-value \
        --secret-id "${aws_secretsmanager_secret.fleet_enroll_secrets[0].id}" \
        --secret-string "$SECRETS_JSON"

      echo "Enroll secrets stored successfully"
    EOF
  }

  depends_on = [
    module.ecs_service_fleet,
    aws_secretsmanager_secret.fleet_enroll_secrets
  ]
}
